...
export function GallerySection() {
  ...
  return (
    <section>
      <Text variant="h2" className="mb-8 text-amber-500">Gallery</Text>
      <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 2xl:grid-cols-8 gap-4">
        { images.map((image, index) => (
          <li
            key={index}
            className="rounded-lg aspect-square bg-center bg-no-repeat bg-cover hover:opacity-80 active:opacity-60 transition-opacity cursor-pointer"
